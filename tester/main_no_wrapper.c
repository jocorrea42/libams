...
void test_strlen(void) {
    ...
    for (int i = 0; tests[i]; i++) {
        ...
        size_t ft = ft_strlen(tests[i]);  // Llamada directa a función asm
...
void test_strcpy(void) {
    ...
    for (int i = 0; tests[i]; i++) {
        ...
        ft_strcpy(dest2, "XXXXXXXXXX");
        char *std = strcpy(dest1, tests[i]);
        char *ft = ft_strcpy(dest2, tests[i]);  // Llamada directa asm
...
void test_strcmp(void) {
    ...
    for (int i = 0; a[i]; i++) {
        ...
        int ft = ft_strcmp(a[i], b[i]);  // Llamada directa asm
...
void test_write(void) {
    ...
    errno = 0;
    ret_ft = ft_write(1, "ft_write: Hola mundo\n", 21);  // Llamada directa asm
    ...
    errno = 0;
    ret_ft = ft_write(-1, "error", 5);  // Llamada directa asm
...
void test_read(void) {
    ...
    lseek(fd, 0, SEEK_SET);
    errno = 0;
    ret_ft = ft_read(fd, buf2, 50);  // Llamada directa asm
    ...
    errno = 0;
    ret_ft = ft_read(-1, buf2, 10);  // Llamada directa asm
...
void test_strdup(void) {
    ...
    for (int i = 0; tests[i]; i++) {
        ...
        char *ft = ft_strdup(tests[i]);  // Llamada directa asm
